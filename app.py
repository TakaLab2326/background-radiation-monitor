...
with tab_ml:
    st.markdown(
        "**課題: 「隠された局」の値を周辺の局から当てる**(欠測補完)。"
        "値の分かっている局をわざと隠して予測し、本当の値と比べて採点します。")
    with st.expander("なぜ「局ごと」に分けて採点するの?(重要)"):
        st.markdown(
            "同じ局のデータが練習問題と試験の両方に入っていると、モデルは答えを丸暗記できて"
            "しまい、見かけの精度が不当に高くなります(リーク)。このアプリは**試験に使う局の"
            "データを練習から完全に除外**する正しい方法で採点します。")
