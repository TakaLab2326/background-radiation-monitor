...
- 収集はGitHub Actionsのみで行う(ローカルでの常時収集は不使用。二重取得を避けるため)
...
| `collect.py` | 1回分の収集(ローカルで試す用。常時収集はGitHub Actions側) |
...
```bash
# 単発収集(ローカルで試す場合。常時収集はGitHub Actionsが10分ごとに実行)
python3 collect.py

# マップ生成
python3 make_map.py && open out/ramis_map.html
```
